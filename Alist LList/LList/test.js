    ...
    describe(name, function () {
        testData.forEach(function (data) {
            ...
            it(`should contain expected data: ${expEls}`, function () {
                ...
                expLList.push.apply(expLList, expEls);
                assert.deepEqual(arr, expLList);
            })
        });

        if (name === 'push') {
            it('should contain expected data and its structure', function () {
                const l = new LList;
                l.push(7);
                assert(l.root === l.last);
                assert.strictEqual(l.root.val, 7);
                assert.isNull(l.root.prev);
                assert.isNull(l.root.next);
            });
            ...
            it('should contain expected data and its structure', function () {
                ...
                l.push(7);
                l.push(1);
                assert.isFalse(l.root === l.last);
                assert.strictEqual(l.root.val, 7);
                assert.isNull(l.root.prev);
                assert.instanceOf(l.root.next, Entry);

                const el2 = l.root.next;
                assert.strictEqual(el2.val, 1);
                assert(el2.prev === l.root);
                assert.isNull(el2.next);
                assert(el2 === l.last);
            });
            ...
            it('should contain expected data and its structure', function () {
                ...
                assert.strictEqual(el2.val, 1);
                assert(el2.prev === l.root);
                assert(l.root.next === el2);

                const el3 = el2.next;
                assert.strictEqual(el3.val, 10);
                assert(el3.prev === el2);
                assert(el3 === l.last);
                assert.isNull(l.last.next);
            });
            ...
            it('should contain expected data and its structure', function () {
                ...
                const el3 = el2.next;
                assert.strictEqual(el3.val, 2);
                assert(el3.prev === el2);
                assert(el2.next === el3);

                const el4 = el3.next;
                assert.strictEqual(el4.val, 10);
                assert(el4.prev === el3);
                assert(el4 === l.last);
                assert.isNull(l.last.next);
            });
...
describe('remove', function () {
    ...
    it(`should delete single element`, function () {
        ...
        assert.strictEqual(arr.root, null)
    });
    ...
    it(`should delete one element`, function () {
        ...
        assert.strictEqual(arr.root.val, 5);
        assert.strictEqual(arr.root.next, null)
    });
    ...
    it(`should delete one element`, function () {
        ...
        assert.strictEqual(arr.length, 1);
        assert.strictEqual(arr.root.val, 10);
        assert.isNull(arr.root.next);
    });
    ...
    it(`should delete one element`, function () {
        ...
        assert.strictEqual(arr.root.val, 10);
        assert.strictEqual(arr.root.next.val, 40);
        assert.isNull(arr.root.next.next)
    })
});
...
describe('splice', function () {
    const testData = [
        ...
        {init: [1], args: [0, 1], expReturn: [1], expLen: 0, expContent: []},
    ...
    testData.forEach(function (data) {
        ...
        it(`should delete elements or add new ones. given data: ${init}, args: ${args}`, function () {
            ...
            assert.deepEqual(returned, expReturnLList, `expected to return ${expReturnLList}, returned ${returned}`);
            const content = new LList;
            content.push.apply(content, expContent);
            assert.deepEqual(arr, content, `resulting data was expected to be ${content}, actual: ${expContent}`);
        })
    })
});
